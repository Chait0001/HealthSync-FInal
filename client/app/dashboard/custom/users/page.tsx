...
import {
  ArrowLeft, Plus, X, ChevronDown, Loader2, UserPlus, Users, AlertCircle,
} from 'lucide-react';
...
const inputCls =
  'w-full px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500/40 focus:border-teal-500 transition';
      ...
      <div className="w-full max-w-lg bg-white dark:bg-[#161b27] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5 sticky top-0 bg-white dark:bg-[#161b27] z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Patient or Doctor accounts only</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            {/* Role: patient or doctor only — admin is intentionally excluded */}
          ...
          {form.role === 'doctor' && (
            <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-white/5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Fees per consultation (₹)">
                  <input
                    className={inputCls}
                    placeholder="e.g. 500"
        ...
        <div className="flex gap-2 justify-end px-6 pb-5">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 transition"
          >
            Cancel
          </button>
          <button
...
export default function CustomUsersPage() {
  ...
  const canView = can('users.view') || can('patients.view') || can('patients.create');
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(res => {
        ...
        // Filter out admin accounts — sub-admin should never see/manage admins
        const nonAdmins = (Array.isArray(data) ? data : []).filter(
          (u: User) => u.role !== 'admin',
        );
        setUsers(nonAdmins);
      })
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {users.length} patient{users.length !== 1 ? 's' : ''} &amp; doctor{users.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
        {canAdd && (
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <input
          className="px-3 py-1.5 text-sm rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/40 w-56 transition"
      ...
      {!loading && filtered.length > 0 && (
        <p className="text-xs text-slate-400 dark:text-slate-500 text-right">
          Showing {filtered.length} of {users.length} users (admin accounts hidden)
        </p>
      )}
    </div>
  );
